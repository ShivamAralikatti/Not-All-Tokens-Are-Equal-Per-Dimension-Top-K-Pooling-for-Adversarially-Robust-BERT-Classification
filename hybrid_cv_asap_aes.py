...
#------
# Top-K avg pooling per dimension model:--
...
def topk_avg_pool_per_dim(
    ...
    if L == 0:
        return torch.zeros(B, H, device=last_hidden.device, dtype=last_hidden.dtype)
...
class R2BERTHybridCLSTopK(nn.Module):
    def __init__(
        self,
        model_name: str,
        top_k_pool: int = 10,
        ...
        self.hidden = hidden
        self.concat_dim = hidden * 2
        self.proj_dim = int(proj_dim) if int(proj_dim) > 0 else 0

        if self.proj_dim > 0:
            self.proj = nn.Sequential(
                nn.LayerNorm(self.concat_dim),
                nn.Dropout(dropout),
                nn.Linear(self.concat_dim, self.proj_dim),
                nn.GELU(),
            )
            head_in = self.proj_dim
        else:
            self.proj = None
            head_in = self.concat_dim

        self.scorer = nn.Linear(head_in, 1)
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor):
        ...
        topk_vec = topk_avg_pool_per_dim(
            h,
            attention_mask,
            k=self.top_k_pool,
        ...
        hybrid = torch.cat([cls_vec, topk_vec], dim=-1) 

        if self.proj is not None:
            rep = self.proj(hybrid)
        else:
            rep = hybrid

        rep = self.dropout(rep)
        z = self.scorer(rep).squeeze(-1)  
...
def main():
    ...
    ap.add_argument("--proj_dim", type=int, default=0,
                    help="0 = no projection (concat dim stays 1536 for bert-base). "
                         "Set 1024 if you want 1536 -> 1024 before the head.")
    ...
    print(f"[Info] repr=Hybrid(CLS || TopKAvgPerDim) | top_k_pool={args.top_k_pool} | exclude_cls_from_topk={not args.include_cls_in_pool}")
    print(f"[Info] proj_dim={args.proj_dim} | dropout={args.dropout}")
    ...
    for pid in prompts:
        ...
        for fold, train_idx, val_idx, test_idx in make_cv_splits(dfp, n_splits=args.cv_folds, seed=args.seed):
            ...
            concat_dim = int(model.concat_dim)
            head_in_dim = int(args.proj_dim) if int(args.proj_dim) > 0 else concat_dim
            ...
            print(f"[Prompt {pid} Fold {fold}] hidden={hidden} | concat_dim={concat_dim} | head_input_dim={head_in_dim}")
            ...
            all_summary_rows.append({
                ...
                "repr_type": "hybrid_cls_topk",
                "bert_hidden": int(hidden),
                "concat_dim": int(concat_dim),
                "proj_dim": int(args.proj_dim),
                "head_input_dim": int(head_in_dim),

                "top_k_pool": int(args.top_k_pool),
                "exclude_cls_from_topk": bool(not args.include_cls_in_pool),
